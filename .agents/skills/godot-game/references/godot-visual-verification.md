...
Use screenshots to confirm runtime state when text logs are not enough.

Note: ONLY WORKS FOR Codex AGENTS.

## Practical Use

- Capture the running Godot window after launching or running a project.
- Inspect UI layout, scene composition, animation state, and obvious rendering failures.
- Use temp captures for Codex-only inspection so the image can be reviewed immediately.

## Workflow Notes

- Keep visual verification tied to a concrete runtime question.
- Use the screenshot helper from the screenshot skill when you need the actual image capture mechanics.
- If screenshots fail on macOS, check Screen Recording permission first.

Follow these save-location rules every time:

1) save to the temp directory.
