...
3) If you need a screenshot for your own inspection, save to the temp directory.
...
The script prints one path per capture. When multiple windows or displays match, it prints multiple paths (one per line) and adds suffixes like `-w<windowId>` or `-d<display>`. After capture, open each saved image with the harness's file-viewing mechanism so the image is available for visual inspection, preferably through inline image attachment or rendering support (for example, an image viewer, `Read`, or any tool that attaches image files inline). Review the images in order, and only manipulate them if needed or requested.

### Workflow examples

- "Take a look at <App> and tell me what you see": capture to temp, then open each printed image path in order for visual inspection.
