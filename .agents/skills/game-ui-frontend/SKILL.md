...
description: Use when the user asks for game HUDs, menus, overlays, responsive layouts, or visual direction that must protect the playfield, especially on browser-based runtime paths.
---
...
Use this skill whenever the game needs a visible interface layer. The job is not to produce generic dashboard UI. The job is to produce a readable, thematic game interface that supports the play experience.

Default assumption for browser tracks: build the game world in canvas or WebGL, and build text-heavy UI in DOM. For non-browser runtimes, preserve the same playfield-protection rules while adapting the implementation to the engine-native UI layer.
...
Use `../game-studio/references/frontend-prompts.md` for concrete prompt shapes.
