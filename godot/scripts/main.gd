...
func _agent_help() -> Dictionary:
	return {
		...
		"endpoints": [
			...
			"GET /stream - SSE event stream for Watch and other agent harnesses",
		...
		"notes": [
			"The registry entry includes stream_url and watch_stream_name so agents do not need to guess ports.",
...
func _write_agent_registry() -> void:
	...
	var data := {
		...
		"base_url": _agent_base_url(),
		"stream_url": _agent_base_url() + "/stream?waking=true",
		"watch_stream_name": "tinygrove:%s:%d" % [agent_profile, agent_http_port],
		"agent_name": OS.get_environment("TINYGROVE_AGENT_NAME").strip_edges(),
		"display_name": _agent_local_display_name(),
		"identity": local_identity,
